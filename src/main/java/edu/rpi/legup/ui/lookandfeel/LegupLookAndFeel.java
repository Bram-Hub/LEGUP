...
public class LegupLookAndFeel extends BasicLookAndFeel {
  /**
   * Return a short string that identifies this look and feel, e.g. "CDE/Motif". This string should
   * be appropriate for a menu item. Distinct look and feels should have different names, e.g. a
   * subclass of MotifLookAndFeel that changes the way a few components are rendered should be
   * called "CDE/Motif My Way"; something that would be useful to a user trying to select a L&amp;F
   ...
   * Presently the well known names are "Motif", "Windows", "Mac", "Metal". Note that a LookAndFeel
   * derived from a well known superclass that doesn't make any fundamental changes to the look or
   ...
   * and Feel". This string is intended for the user, e.g. in the title of a window or in a ToolTip
   * message.
   *
  ...
  /**
   * Return {@code true} if the underlying platform supports and or permits this look and feel. This
   * method returns {@code false} if the look and feel depends on special resources or legal
   ...
   * is invoked before the {@code UIManager} invokes {@code getDefaults}. This method is intended to
   * perform any initialization for the look and feel. Subclasses should do any one-time setup they
   * need here, rather than in a static initializer, because look and feel class objects may be
   * loaded just to discover that {@code isSupportedLookAndFeel()} returns {@code false}.
   *
   * @see #uninitialize
   * @see UIManager#setLookAndFeel
   */
  ...
  /**
   * Populates {@code table} with mappings from {@code uiClassID} to the fully qualified name of the
   * ui class. The value for a particular {@code uiClassID} is {@code "javax.swing.plaf.basic.Basic
   * + uiClassID"}. For example, the value for the {@code uiClassID} {@code TreeUI} is {@code
  ...
  protected void initComponentDefaults(UIDefaults table) {
    ...
    table.put(
        ...
        BorderFactory.createCompoundBorder(
            MaterialBorders.LIGHT_LINE_BORDER, BorderFactory.createEmptyBorder(0, 5, 0, 0)));
    ...
    table.put(
        "RadioButtonMenuItem.selectedCheckIcon", new ImageIcon(MaterialImages.RADIO_BUTTON_ON));
    ...
    table.put(
        "CheckBoxMenuItem.selectedCheckIcon", new ImageIcon(MaterialImages.PAINTED_CHECKED_BOX));
