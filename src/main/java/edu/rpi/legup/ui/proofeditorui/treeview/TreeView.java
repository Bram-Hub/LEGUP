...
public class TreeView extends ScrollView implements ITreeListener {
      ...
      for (TreeNode node : trans.getParents()) {
        ...
        for (TreeNode ancestor : ancestors) {
          ...
          for (PuzzleElement pelement :
            ...
            boolean modifiable =
                tree.getRootNode().getBoard().getPuzzleElement(oldElement).isModifiable();
    ...
    if (!children.isEmpty()) {
      ...
      if (rule instanceof CaseRule) {
        ...
        for (TreeNode ancestor : ancestors) {
          ...
          for (PuzzleElement pelement :
              caseRule.dependentElements(node.getBoard(), children.get(0).getSelection())) {
            // decrement, unlock if 0 cases depended
            PuzzleElement oldElement = ancestor.getParent().getBoard().getPuzzleElement(pelement);
            oldElement.setCasesDepended(oldElement.getCasesDepended() - 1);
            if (oldElement.getCasesDepended() == 0) {
      ...
      default:
        {
          ...
          for (int i = 0; i < children.size(); i++) {
            ...
            childView.setEndX((NODE_GAP_WIDTH + DIAMETER) * (depth + 1) + RADIUS - TRANS_GAP / 2);
            childView.setEndY(
                yLoc - (int) (nodeView.getSpan() / 2) + span + (int) (childView.getSpan() / 2));
  ...
  /**
   * Calculates the sub span of a given sub tree rooted at the specified view and stops at the tree
   * puzzleElement view specified as stop. Stop tree puzzleElement is NOT included in the span
   * calculation
   *
   * @param view
   * @param stop
  ...
  /**
   * Reorders branches such that merging branches are sequentially grouped together and transitions
   * are kept in relative order in the list of child transitions of the specified node
   *
   * @param node root node of the branches
   * @param branches DisjointSets of the child branches of the specified node which determine which
   *     branches merge
   */
