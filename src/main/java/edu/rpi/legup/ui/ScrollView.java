...
public class ScrollView extends JScrollPane {
  ...
  /**
   * ScrollView Constructor - creates a ScrollView object using the controller handle the ui events
   *
  ...
  public void zoomTo(double newScale) {
    ...
    Point p =
        new Point(
            viewport.getWidth() / 2 + viewport.getX(), viewport.getHeight() / 2 + viewport.getY());
