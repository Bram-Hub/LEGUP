...
public class ApplyDefaultDirectRuleCommand extends PuzzleCommand {
  ...
  /**
   * ApplyDefaultDirectRuleCommand Constructor creates a command for applying the default of a basic
   * rule
   *
          ...
          if (!node.getChildren().isEmpty()) {
            ...
            if (rule.getDefaultBoard(node) == null) {
              return CommandError.DEFAULT_APPLICATION
                  + " - "
                  + "This selection contains a tree element that this rule cannot be applied to.";
            }
          }
        } else {
          return CommandError.DEFAULT_APPLICATION
