...
class ShortTruthTableImporter extends PuzzleImporter {
  ...
  /**
   * Parse a string into all the cells, the y position of the statement is passed so the y position
   * can be set
   *
   * @param statement
   * @param y
  ...
  private List<ShortTruthTableCell> getCells(String statement, int y) {
    ...
    for (int i = 0; i < statement.length(); i++) {
      ...
      ShortTruthTableCell cell =
          new ShortTruthTableCell(c, ShortTruthTableCellType.getDefaultType(c), new Point(i, y));
   ...
   * cells are set to UNKNWON, it their value is given, it will be set later in the import process.
   *
  ...
  private ShortTruthTableBoard generateBoard(
    ...
    ShortTruthTableBoard sttBoard =
        new ShortTruthTableBoard(
            width, height, statements.toArray(new ShortTruthTableStatement[statements.size()]));
  ...
  private void setGivenCells(
    ...
    for (int i = 0; i < cellData.getLength(); i++) {
      ...
      ShortTruthTableCell cell =
          (ShortTruthTableCell) puzzle.getFactory().importCell(cellData.item(i), sttBoard);
