    ...
    for (int i = -1; i < 2; i += 2) {
      for (int j = -1; j < 2; j += 2) {
        ...
        if (cornerType == NurikabeType.WHITE || cornerType == NurikabeType.NUMBER) {
          ...
          // 2. Check if the intersecting adjacent spaces of the white space and the black corner
          // are empty
          if (board.getCell(cornerLocation.x, cellLocation.y).getType() == NurikabeType.UNKNOWN
            ...
            modified
                .getCell(cornerLocation.x, cellLocation.y)
                .setData(NurikabeType.BLACK.toValue());
            modified
                .getCell(cellLocation.x, cornerLocation.y)
                .setData(NurikabeType.BLACK.toValue());
            ...
            if (containsContradiction) {
              ...
              // If the region size is 0, there is a possibility that there was only 1 cell in the
              // white
              ...
              // If the region size is not 0, make sure the regionNumber and the region size match
              // (need
