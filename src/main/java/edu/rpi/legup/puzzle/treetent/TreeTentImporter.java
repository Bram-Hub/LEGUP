  ...
  public void initializeBoard(Node node) throws InvalidFileFormatException {
    try {
      ...
      for (int i = 0; i < elementDataList.getLength(); i++) {
        TreeTentCell cell =
            (TreeTentCell) puzzle.getFactory().importCell(elementDataList.item(i), treeTentBoard);
      ...
      if (eastClues.getLength() != treeTentBoard.getHeight()
        ...
        throw new InvalidFileFormatException(
            "TreeTent Importer: there must be same number of clues as the dimension of the board");
      }
      ...
      if (boardElement.getElementsByTagName("lines").getLength() == 1) {
        ...
        for (int i = 0; i < linesList.getLength(); i++) {
          treeTentBoard
              .getLines()
              .add((TreeTentLine) puzzle.getFactory().importCell(linesList.item(i), treeTentBoard));
        }
      }
