...
public class FillinRowCaseRule extends CaseRule {
  ...
  public CaseBoard getCaseBoard(Board board) {
    ...
    for (PuzzleElement element : clues) {
      ...
      // treeTentBoard.getRowCol(((TreeTentCell)element).getLocation().y, TreeTentType.UNKNOWN,
      // true).size() != 0) ||
      //     (((TreeTentCell) element).getType() == TreeTentType.CLUE_EAST &&
      // treeTentBoard.getRowCol(((TreeTentCell)element).getLocation().x, TreeTentType.UNKNOWN,
      // false).size() != 0)) {
  ...
  private ArrayList<Board> genCombinations(
      TreeTentBoard iBoard, List<TreeTentCell> tiles, int target, Integer index, boolean isRow) {
