...
public class TentForTreeDirectRule extends DirectRule {
    ...
    public TentForTreeDirectRule() {
        super(
                "TREE-BASC-0006",
                "Tent for Tree",
                "If only one unlinked tent and no blank cells are adjacent to an unlinked tree, the"
                        + " unlinked tree must link to the unlinked tent.",
    ...
    public String checkRuleRawAt(TreeTransition transition, PuzzleElement puzzleElement) {
        ...
        if (line.getC1().getType() == TreeTentType.TREE
                && line.getC2().getType() == TreeTentType.TENT) {
            tree = line.getC1();
            tent = line.getC2();
        } else {
            if (line.getC2().getType() == TreeTentType.TREE
                    && line.getC1().getType() == TreeTentType.TENT) {
                tree = line.getC2();
                tent = line.getC1();
            } else {
                return super.getInvalidUseOfRuleMessage()
                        + ": This line must connect a tree to a tent.";
            }
        }
        ...
        if (forced == 1) {
            ...
            if (forced == -1) {
                ...
                if (forced == -2) {
                    ...
                    return super.getInvalidUseOfRuleMessage()
                            + ": This tree and tent don't need to be linked.";
                }
            }
        }
    }
