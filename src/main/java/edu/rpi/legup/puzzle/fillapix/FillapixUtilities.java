...
public class FillapixUtilities {
  ...
  /**
   * Gets all cells that are contained in the square defined as having 'distance' cells between the
   * center and the outer wall. For example, distance = 1:
   *
   * <p>|X|X|X|X|X|
   *
   * <p>|X| | | |X|
   ...
   *     combination. Each Boolean array will be <code>totalNumItems</code> long and each index will
   *     be <code>true</code> if the corresponding item is included in that combination, and <code>
