...
public class UnresolvedNumberContradictionRule extends ContradictionRule {
  ...
  public UnresolvedNumberContradictionRule() {
    super(
        ...
        // specify a number? defaulting to every number for now. expand to more than
        // duplicate?
  ...
  public String checkContradictionAt(Board board, PuzzleElement puzzleElement) {
    ...
    for (int i = 0; i < skyscrapersBoard.getWidth(); i++) {
      ...
      for (SkyscrapersCell presentCell :
          skyscrapersBoard.getRowCol(loc.y, SkyscrapersType.Number, true)) {
      ...
      if (!exists) {
        ...
        if (caseRule.getCasesFor(board, skyscrapersBoard.getWestClues().get(loc.y), num).size()
            == 0) {
          return null;
        }
      }
      ...
      exists = false;
      for (SkyscrapersCell presentCell :
          skyscrapersBoard.getRowCol(loc.x, SkyscrapersType.Number, false)) {
        if (presentCell.getData() == num) {
          exists = true;
          break;
        }
      ...
      if (!exists) {
        if (caseRule.getCasesFor(board, skyscrapersBoard.getNorthClues().get(loc.x), num).size()
            == 0) {
          return null;
        }
      }
