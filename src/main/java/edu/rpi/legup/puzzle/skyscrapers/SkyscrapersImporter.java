    ...
    for (int y = 0; y < size; y++) {
      for (int x = 0; x < size; x++) {
        SkyscrapersCell cell =
            new SkyscrapersCell(SkyscrapersType.UNKNOWN.toValue(), new Point(x, y), size);
  ...
  public void initializeBoard(Node node) throws InvalidFileFormatException {
    try {
      ...
      for (int i = 0; i < elementDataList.getLength(); i++) {
        SkyscrapersCell cell =
            (SkyscrapersCell)
                puzzle.getFactory().importCell(elementDataList.item(i), skyscrapersBoard);
      ...
      for (int y = 0; y < size; y++) {
        for (int x = 0; x < size; x++) {
          if (skyscrapersBoard.getCell(x, y) == null) {
            SkyscrapersCell cell =
                new SkyscrapersCell(SkyscrapersType.UNKNOWN.value, new Point(x, y), size);
            cell.setIndex(y * size + x);
            cell.setModifiable(true);
            skyscrapersBoard.setCell(x, y, cell);
          }
      ...
      if (eastClues.getLength() != skyscrapersBoard.getHeight()
        ...
        throw new InvalidFileFormatException(
            "Skyscraper Importer: there must be same number of clues as the dimension of the board");
      }
      ...
      for (int i = 0; i < eastClues.getLength(); i++) {
        ...
        skyscrapersBoard
            .getWestClues()
            .set(/*index - 1*/ i, new SkyscrapersClue(index, i, SkyscrapersType.CLUE_WEST));
        skyscrapersBoard
            .getEastClues()
            .set(/*index - 1*/ i, new SkyscrapersClue(value, i, SkyscrapersType.CLUE_EAST));
      }
      ...
      for (int i = 0; i < southClues.getLength(); i++) {
        ...
        skyscrapersBoard
            .getNorthClues()
            .set(/*index - 1*/ i, new SkyscrapersClue(index, i, SkyscrapersType.CLUE_NORTH));
        skyscrapersBoard
            .getSouthClues()
            .set(/*index - 1*/ i, new SkyscrapersClue(value, i, SkyscrapersType.CLUE_SOUTH));
      }
