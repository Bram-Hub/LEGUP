...
public class LastSingularCellDirectRule extends DirectRule {
    ...
    public LastSingularCellDirectRule() {
        super(
                ...
                "There is only one cell on this row/col for this number that does not create a"
    ...
    public String checkRuleRawAt(TreeTransition transition, PuzzleElement puzzleElement) {
        SkyscrapersBoard initialBoard =
                (SkyscrapersBoard) transition.getParents().get(0).getBoard();
        ...
        ArrayList<Board> XCandidates =
                caseRule.getCasesFor(
                        initialBoard,
                        initialBoard.getWestClues().get(finalCell.getLocation().y),
                        (Integer) finalCell.getData());
        ArrayList<Board> YCandidates =
                caseRule.getCasesFor(
                        initialBoard,
                        initialBoard.getNorthClues().get(finalCell.getLocation().x),
                        (Integer) finalCell.getData());
    ...
    private String candidateCheck(
            ArrayList<Board> candidates, PuzzleElement puzzleElement, SkyscrapersCell finalCell) {
        if (candidates.size() == 1) {
            if (((SkyscrapersCell) candidates.get(0).getPuzzleElement(puzzleElement)).getType()
                    == SkyscrapersType.Number) {
                if (candidates.get(0).getPuzzleElement(puzzleElement).getData()
                        == finalCell.getData()) {
                    return null;
                }
