...
public class FinishWithBombsDirectRule extends DirectRule {
    ...
    public String checkRuleRawAt(TreeTransition transition, PuzzleElement puzzleElement) {
        ...
        if (!(parentCell.getTileType() == MinesweeperTileType.FLAG
                && cell.getTileType() == MinesweeperTileType.FLAG)) {

            return super.getInvalidUseOfRuleMessage()
                    + ": This cell must be empty to be applicable with this rule.";
        }

        if (MinesweeperUtilities.surroundingMustBeBombs(parentBoard, cell)) {

            return null;
        } else {
            return super.getInvalidUseOfRuleMessage() + ": The surrounding unset cells are not forced to be bombs";
        }
    }
