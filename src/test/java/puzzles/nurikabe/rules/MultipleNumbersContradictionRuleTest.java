...
public class MultipleNumbersContradictionRuleTest {
    ...
    private static final MultipleNumbersContradictionRule RULE =
            new MultipleNumbersContradictionRule();
    ...
    public void MultipleNumbersContradictionRule_TwoSurroundBlackTest()
        ...
        TestUtilities.importTestBoard(
                "puzzles/nurikabe/rules/MultipleNumbersContradictionRule/MultipleNumbers",
                nurikabe);
    ...
    /**
     * Tests the Multiple Numbers contradiction rule for a more complex regions with multiple
     * numbers
     */
    @Test
    public void MultipleNumbersContradictionRule_ComplexRegion() throws InvalidFileFormatException {
        TestUtilities.importTestBoard(
                "puzzles/nurikabe/rules/MultipleNumbersContradictionRule/ComplexRegion", nurikabe);
        TreeNode rootNode = nurikabe.getTree().getRootNode();
        TreeTransition transition = rootNode.getChildren().get(0);
        transition.setRule(RULE);
    ...
    /**
     * Tests the Multiple Numbers contradiction rule for two regions with one number each, separated
     * diagonally
     */
    @Test
    public void MultipleNumbersContradictionRule_FalseContradiction()
            throws InvalidFileFormatException {
        TestUtilities.importTestBoard(
                "puzzles/nurikabe/rules/MultipleNumbersContradictionRule/FalseContradiction",
                nurikabe);
        TreeNode rootNode = nurikabe.getTree().getRootNode();
        TreeTransition transition = rootNode.getChildren().get(0);
        transition.setRule(RULE);
