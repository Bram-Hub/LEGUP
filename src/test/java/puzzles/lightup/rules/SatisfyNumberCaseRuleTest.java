...
public class SatisfyNumberCaseRuleTest {
  ...
  // creates two boards for what is expected output, and checks that the getcases function produces
  // the correct boards
  ...
  public void SatisfyNumberTest() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/lightup/rules/SatisfyNumberCaseRule/SatisfyNumber", lightUp);
    ...
    ArrayList<Board> cases =
        RULE.getCases(b, numbered_cell); // C MUST BE THE NUMBERED TILE, NOT ANY RANDOM EMPTY ONE
  ...
  public void SatisfyNumberTestTwo() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/lightup/rules/SatisfyNumberCaseRule/SatisfyNumberTwo", lightUp);
    TreeNode rootNode = lightUp.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
