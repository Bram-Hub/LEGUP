...
public class NoTentForTreeContradictionRuleTest {
  ...
  /**
   * @throws InvalidFileFormatException Tests if a tree is next to only grass in a 2x2 grid triggers
   *     the contradiction
   */
  @Test
  ...
  public void NoTentForTreeContradictionRule_Diagonal() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/treetent/rules/NoTentForTreeContradictionRule/NoTentForTreeDiagonal", treetent);
    TreeNode rootNode = treetent.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
  ...
  public void NoTentForTreeContradictionRule_TwoTrees() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/treetent/rules/NoTentForTreeContradictionRule/NoTentForTreeTwoTrees", treetent);
    TreeNode rootNode = treetent.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
