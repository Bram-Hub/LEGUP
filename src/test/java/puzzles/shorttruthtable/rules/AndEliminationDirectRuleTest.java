...
public class AndEliminationDirectRuleTest {
   ...
   * both B and C are true and asserts that each one of them is not a valid application of the rule.
   *
  ...
  public void trueAndTest1() throws InvalidFileFormatException {
    ...
    ShortTruthTableCellType[] cellTypes = {
      ShortTruthTableCellType.TRUE, ShortTruthTableCellType.FALSE, ShortTruthTableCellType.UNKNOWN
    };
   ...
   * Given one statement: B^C where ^ is true
   *
   * <p>Checks all possible combinations of true and unknown for B and C except for where both B and
   * C are unknown and asserts that each one of them is a valid application of the rule.
   *
   * @throws InvalidFileFormatException
   */
  @Test
  ...
  public void falseAndWithKnownFalseTest() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/shorttruthtable/rules/AndEliminationDirectRule/FalseAndWithKnownFalse", stt);
    TreeNode rootNode = stt.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
  ...
  public void falseAndWithKnownTrueTest() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/shorttruthtable/rules/AndEliminationDirectRule/FalseAndWithKnownTrue", stt);
    TreeNode rootNode = stt.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
