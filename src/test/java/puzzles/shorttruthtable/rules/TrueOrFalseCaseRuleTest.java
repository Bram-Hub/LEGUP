...
public class TrueOrFalseCaseRuleTest {
  ...
  /**
   * Tests the True or False case rule by ensuring that it results in two children, one that
   * contains Statement as true and one that contains Statement as false.
   */
  @Test
  public void TwoBranchesTest() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/shorttruthtable/rules/TrueOrFalseCaseRule/Statement", stt);
    ...
    Assert.assertTrue(
        cellType1.equals(ShortTruthTableCellType.TRUE)
            || cellType1.equals(ShortTruthTableCellType.FALSE));
    Assert.assertTrue(
        cellType2.equals(ShortTruthTableCellType.TRUE)
            || cellType2.equals(ShortTruthTableCellType.FALSE));
