...
public class BiconditionalEliminationTest {
   ...
   *
   * <p>Asserts that setting any combination of A and B at the same time is not a valid application
   * of this rule
   *
   * @throws InvalidFileFormatException
   */
  ...
  public void TrueBiconditionalSetBothAtOnceTest() throws InvalidFileFormatException {
    TestUtilities.importTestBoard(
        "puzzles/shorttruthtable/rules/BiconditionalEliminationDirectRule/TrueBiconditional", stt);
    TreeNode rootNode = stt.getTree().getRootNode();
    TreeTransition transition = rootNode.getChildren().get(0);
    transition.setRule(RULE);
  ...
  /**
   * Asserts that setting any combination of A and B at the same time is not a valid application of
   * this rule. This is tested on multiple files.
   *
   * @throws InvalidFileFormatException
   */
  @Test
